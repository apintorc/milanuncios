...
public class AnuncioController {
    ...
    public ResponseEntity<List<AnuncioDto>> get_anuncios_by_user(@PathVariable("user") String user){
		return ResponseEntity.ok(anuncioService.getAnunciosByUser(user));
	}
	...
	public ResponseEntity<Integer>add_anuncio(@RequestBody AnuncioDto anunciodtoAnuncioDto){
		...
		if(anuncio==null) {
	    	...
	    	int idAleatorio = random.nextInt(Integer.MAX_VALUE);
	    	anunciodtoAnuncioDto.setId_anuncio(idAleatorio);
	    	anunciodtoAnuncioDto.setFecha_publicacion(new Date());
	    	anunciodtoAnuncioDto.setUser("User1");
